...
public class PaneImplement {
            ...
            scene.setOnKeyPressed(e -> {
                if (e.getCode() == KeyCode.ESCAPE) {
                    ...
                    try {
                        ...
                        // 如果取消失败，尝试强制关闭窗口
                        try {
                            if (captureStage != null) captureStage.close();
        ...
        try {
            ...
            if (captureStage != null) {
                ...
                try {
                    ...
                    debugLog("关闭所有者窗口时出错: " + e.getMessage());
                }
            } else {
    ...
    private String getResizeDirection(double x, double y) {
        if (selectionRect == null) return "none";
    ...
    private void updateSelectionMask() {
        if (selectionRect == null || overlayRect == null) return;
    ...
    private void updateBorderHandles() {
        ...
        // 只有在选择框大小足够大时才显示控制点
        if (rectWidth < 20 || rectHeight < 20) return;
    ...
    private void saveImageAs(Image image, Stage owner) {
        try {
            ...
            FileChooser.ExtensionFilter jpgFilter = 
                new FileChooser.ExtensionFilter("JPEG 图像", "*.jpg", "*.jpeg");
    ...
    private void showSticker(WritableImage image) {
        ...
        try {
            ...
            // 使用最高质量的图像显示
            imageView.getProperties().put("javafx.scene.canvas.width", image.getWidth());
            imageView.getProperties().put("javafx.scene.canvas.height", image.getHeight());
            ...
            imageContainer.setMaxWidth(width);
            imageContainer.setMaxHeight(height);
            imageContainer.setMinWidth(width);
            imageContainer.setMinHeight(height);

            // 添加边框 - 简化边框设计，避免过多的嵌套元素
        Rectangle border = new Rectangle(width, height);
        border.setFill(null);
            border.setStroke(Color.DODGERBLUE.deriveColor(0, 1, 1, 0.7));
        border.setStrokeWidth(1.5);
            border.setArcWidth(2);
            border.setArcHeight(2);
            
            // 创建外部辉光边框 - 显示蓝色光晕效果
            Rectangle glowBorder = new Rectangle(width + 10, height + 10);
            glowBorder.setFill(null);
            glowBorder.setStroke(Color.DODGERBLUE);
            glowBorder.setStrokeWidth(4.0);
            glowBorder.setArcWidth(5);
            glowBorder.setArcHeight(5);
            glowBorder.setTranslateX(-5); // 居中调整
            glowBorder.setTranslateY(-5); // 居中调整
            glowBorder.setOpacity(0.6);
            
            // 创建闪烁的蓝色阴影效果
            javafx.scene.effect.DropShadow blueShadow = new javafx.scene.effect.DropShadow();
            blueShadow.setRadius(15.0);
            blueShadow.setOffsetX(0);
            blueShadow.setOffsetY(0);
            blueShadow.setColor(Color.DODGERBLUE);
            blueShadow.setSpread(0.7); // 较高的spread值让阴影更明显
            border.setEffect(blueShadow);
            
            // 创建内部边框闪烁动画效果
            javafx.animation.FadeTransition shadowPulse = new javafx.animation.FadeTransition(
                    javafx.util.Duration.seconds(0.5), border);
            shadowPulse.setFromValue(0.5);
            shadowPulse.setToValue(1.0);
            shadowPulse.setCycleCount(6); // 0.5秒一次，6次共3秒
            shadowPulse.setAutoReverse(true);
            
            // 创建外部辉光边框闪烁动画
            javafx.animation.FadeTransition glowPulse = new javafx.animation.FadeTransition(
                    javafx.util.Duration.seconds(0.5), glowBorder);
            glowPulse.setFromValue(0.3);
            glowPulse.setToValue(0.8);
            glowPulse.setCycleCount(6); // 与内部边框动画同步
            glowPulse.setAutoReverse(true);
            
            // 创建同时运行两个闪烁效果的并行动画
            javafx.animation.ParallelTransition parallelPulse = new javafx.animation.ParallelTransition(
                    shadowPulse, glowPulse);
            
            // 创建一个顺序动画，在闪烁结束后切换到普通阴影效果
            ...
            normalShadow.setRadius(10.0);
            normalShadow.setOffsetX(3.0);
            normalShadow.setOffsetY(3.0);
            normalShadow.setColor(Color.color(0, 0, 0, 0.5));
            
            // 添加闪烁动画到序列
            sequence.getChildren().add(parallelPulse);
            
            // 添加动画结束的处理
            sequence.setOnFinished(e -> {
                // 切换到普通阴影效果
                border.setEffect(normalShadow);
                border.setStroke(Color.DARKBLUE.deriveColor(0, 1, 1, 0.5));
                border.setStrokeWidth(1.0);
                
                // 移除闪烁的外部边框
                Platform.runLater(() -> {
                    if (imageContainer.getChildren().contains(glowBorder)) {
                        imageContainer.getChildren().remove(glowBorder);
                    }
                });
                
                debugLog("闪烁动画结束，切换到普通阴影效果");
            });
            
            // 确保imageView自身不受边框效果影响
            imageView.setEffect(null);
            
            // 清空已有子元素并添加新元素
            imageContainer.getChildren().clear();
            imageContainer.getChildren().addAll(glowBorder, border, imageView);
        ...
        stickerStage.setAlwaysOnTop(true);
            ...
            // 确保舞台本身的透明度为100%
            stickerStage.setOpacity(1.0);
            
            // 为贴图窗口添加ESC键处理
            scene.setOnKeyPressed(e -> {
                if (e.getCode() == KeyCode.ESCAPE) {
                    debugLog("贴图窗口中ESC键被按下 - 关闭贴图");
        ...
        ContextMenu contextMenu = new ContextMenu();
            ...
            imageContainer.setOnMouseExited(event -> {
                ...
                if (contextMenu.isShowing()) {
                    ...
                    new Thread(() -> {
                        try {
                            Thread.sleep(300);  // 300毫秒延迟
                            Platform.runLater(() -> {
                                // 再次检查是否应该关闭，避免在菜单上操作时关闭菜单
